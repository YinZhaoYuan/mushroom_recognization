...
static void train(const std::string& train_images_file,
	...
	size_t imgResizeWidth)
{
	...
	success = load_images(train_images_file, images, labels, imgResizeChannels, imgResizeHeight, imgResizeWidth);
	...
	//train
	float validateRate = 0.9;
	std::vector<image_t> train_images(static_cast<size_t>(images.size()*validateRate));
	std::vector<label_t> train_labels(static_cast<size_t>(labels.size()*validateRate));
...
static void test(const std::string& test_images_path,
	const std::string& modelFilePath,
	size_t imgResizeChannels,
	size_t imgResizeHeight,
	size_t imgResizeWidth
	)
{
	...
	std::vector<image_t> images;
	std::vector<label_t> labels;
	success = load_images(test_images_path, images, labels, imgResizeChannels, imgResizeHeight, imgResizeWidth);
	assert(success && images.size() > 0 && labels.size() > 0);
	assert(images.size() == labels.size());
